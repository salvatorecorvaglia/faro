...
describe('useConnections', () => {
  ...
  it('surfaces a failed refresh instead of spinning forever', async () => {
    // `refresh` fetches both in one `Promise.all`, so the keychain probe has to
    // be declared even though this test is about the other one failing.
    mockInvoke({
      list_connection_status: faroError('store', 'settings are unreadable'),
      keychain_available: () => true,
    });
