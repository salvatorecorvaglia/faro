...
import { addRow, emptyEdits } from '@/lib/edits';
...
describe('ResultGrid keyboard and semantics', () => {
  const columns = [
    {
      name: 'id',
      typeName: 'int4',
  ...
  it('clears the selection on Escape', () => {
    ...
    // Nothing else in the grid claims Escape, and a highlight with no way to
    // clear it sits there after the user has moved on.
  ...
  it('moves the selection with the arrow keys', () => {
    ...
    // Still exactly one cell selected, not two.
    expect(selectedCells()).toHaveLength(1);
  });

  it('counts staged inserts in aria-rowcount', () => {
    // A screen reader told there are 2 rows while 3 are navigable announces
    // the last one as being outside the table.
    const edits = addRow(emptyEdits(), columns);
    renderGrid({
      editing: {
        edits,
        onCellEdit: vi.fn(),
  ...
  it('gives a staged insert the same row semantics as a fetched row', () => {
    const edits = addRow(emptyEdits(), columns);
    renderGrid({
      editing: {
        edits,
    ...
    const indexes = rows.map((r) => r.getAttribute('aria-rowindex'));
    // Continues the numbering rather than restarting or going unlabelled.
...
describe('ResultGrid', () => {
  ...
  // An empty table and a table whose rows a filter excluded are different
  // facts, and the grid used to state the second one for both — telling
  // someone looking at an empty table that a filter they never set hid its
  // rows.
  it('says the result is empty when nothing is filtered', () => {
    renderGrid({ result: makeResult({ rows: [] }), filters: [] });
  ...
  it('blames the filters when there are some', () => {
    renderGrid({
      ...
      filters: [{ column: 'name', op: 'contains', value: 'zzz' }],
    });
