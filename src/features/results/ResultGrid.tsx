...
export function ResultGrid({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: keyed on result.columns on purpose; see above
  useEffect(() => {
  ...
  const widthOf = useCallback(
    (i: number) => widths[columnKeys[i] ?? ''] ?? DEFAULT_COL_WIDTH,
    [widths, columnKeys],
  );
  ...
  function onGridKeyDown(e: React.KeyboardEvent) {
    ...
    switch (e.key) {
      ...
      case 'PageUp':
        ...
        break;
      case 'Escape':
        // Nothing else in the grid claims Escape, and a selected cell with no
        // way to unselect it leaves the highlight sitting there after the user
        // has moved on.
        if (selected) {
          e.preventDefault();
          setSelected(null);
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="grid"
      // Staged inserts are rows of this grid too, and a screen reader that is
      // told there are 20 rows while 22 are navigable reports the last two as
      // being outside the table.
      aria-rowcount={result.rows.length + (editing?.edits.inserts.length ?? 0)}
    ...
    >
      <div style={{ width: totalWidth, minWidth: '100%' }}>
            ...
            {visible.map((ci) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className="truncate text-2xs font-normal"
                    style={{ color: 'var(--text-faint)' }}
                  >
                    {col.typeName}
                  </span>
            ...
            <div className="flex" style={{ height: FILTER_HEIGHT }}>
              ...
              {visible.map((ci) => {
                ...
                return (
                  ...
                  >
                    <select
                      className="shrink-0 bg-transparent text-2xs outline-none"
        ...
        <div style={{ height: rowVirtualizer.getTotalSize(), position: 'relative' }}>
          {rowVirtualizer.getVirtualItems().map((v) => {
            ...
            return (
              ...
              >
                <div
                  className="group/num flex shrink-0 items-center justify-end gap-1 border-r px-1.5 text-xs tabular-nums"
                  style={{
                    width: ROW_NUM_WIDTH,
                    borderColor: 'var(--border)',
        ...
        {editing?.edits.inserts.map((row, insertIndex) => (
          <div
            key={row.key}
            role="row"
            // Continues the numbering from the fetched page, which is where
            // these sit visually.
            aria-rowindex={result.rows.length + insertIndex + 1}
            className="flex"
            style={{
              height: ROW_HEIGHT,
              width: totalWidth,
          ...
          >
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  // Any open editor is addressed by insert *index*, which the
                  // removal is about to shift; closing it first is both correct
                  // and what the user expects from deleting the row.
                  setEditingCell(null);
                  editing.onRemoveInsert(insertIndex);
                }}
              >
                <IconTrash size={10} />
              </button>
      ...
      {result.rows.length === 0 && (
        <div
          ...
          style={{ color: 'var(--text-faint)' }}
        >
          {/* An empty table and a filtered-out table are different facts. Saying
              "no rows match" when no filter is set asserts that something
              excluded them, which sends the user looking for a filter that was
              never there. */}
