...
export function ResultPanel(props: ResultPanelProps) {
  ...
  // `statements` is the trigger, not an input — the body reads none of it. A
  // new query is exactly when a client-side sort or filter has to be dropped,
  // and removing the dependency would leave the old one applied to unrelated
  // columns.
        ...
        {rawRows && (
          <div className="flex items-center gap-0.5">
            <button
              ...
              // These three signalled their state with colour alone, which a
              // screen reader cannot see and a colour-blind user may not
              // either.
            ...
            >
              <IconFilter size={12} />
              {filters.length > 0 && (
                <span className="text-2xs tabular-nums">{filters.length}</span>
              )}
            </button>
            <button
              type="button"
...
function JsonView({ result }: { result: ResultSet }) {
  ...
  return (
    <div className="h-full overflow-auto">
      <pre className="selectable p-3 font-mono text-sm leading-relaxed">{text}</pre>
      {result.rows.length > LIMIT && (
        <p className="px-3 pb-3 text-xs" style={{ color: 'var(--text-faint)' }}>
...
function CellInspector({
  ...
  return (
    ...
    >
      ...
      <pre className="selectable min-h-0 flex-1 overflow-auto px-2.5 pb-2.5 font-mono text-sm">
        {pretty}
      </pre>
    </div>
  );
