  ...
  return (
    <Modal open={open} onClose={onClose} title="Export" width={440}>
      <div className="flex flex-col gap-3">
        ...
        {/* A shared `name` is what makes these one control: without it the two
            are unrelated radios, arrow keys do not move between them, and a
            screen reader announces no group. */}
        {canExportTable && (
          <div className="flex flex-col gap-1.5" role="radiogroup" aria-label="Export scope">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
                name="export-scope"
                checked={wholeTable}
                onChange={() => setWholeTable(true)}
              />
              Entire table
            </label>
            <label className="flex items-center gap-2 text-sm">
        ...
        {done && (
          <p className="break-all text-xs" style={{ color: 'var(--success)' }}>
            {done}
          </p>
        )}
