...
export function TableTab({ tab }: { tab: Tab }) {
  ...
  /**
   * Generation counter for `describeTable`.
   *
   * `load` already refuses a reply a newer request has superseded; the describe
   * running beside it had no such guard, so two quick table switches could
   * leave `detail` belonging to the *previous* table — and `detail.primaryKey`
   * decides both whether the grid is editable and what `toPendingChanges`
   * anchors its WHERE clauses on.
   */
  const describeSeq = useRef(0);

  // Runs once per table, keyed on its identity rather than on the `tab` object
  // or on `load`. Sort, filter and paging each re-run `load` explicitly, so
  // adding them here would reload the page twice for every one of those.
  // biome-ignore lint/correctness/useExhaustiveDependencies: once per table; see above
  useEffect(() => {
    if (!tab.connectionId || !tab.table) return;

    // A new table invalidates the old one's sort and filters. They used to
    // survive here while the `load` below deliberately started clean, so the
    // filter boxes went on showing the previous table's filters over data that
    // was never filtered by them — and the next sort click sent them onward.
    setSort(null);
    setFilters([]);
    ...
    ipc
      ...
      .then((d) => {
        if (describeSeq.current === seq) setDetail(d);
      })
      .catch(() => {
        if (describeSeq.current === seq) setDetail(null);
      });

    // `load` records its own failures on the tab.
    void load(0, null, []);
  ...
  // Memoized: a fresh object literal on every render would change identity each
  // time a single cell is edited, re-rendering the whole grid underneath.
  const browseResult = tab.browseResult;
      ...
      editable
        ? {
            ...
            onCellEdit: (rowIndex: number, column: string, value: EditValue) => {
              const columnIndex = browseResult?.columns.findIndex((c) => c.name === column) ?? -1;
              const original =
                columnIndex >= 0 ? browseResult?.rows[rowIndex]?.[columnIndex] : undefined;
        ...
        : undefined,
    [editable, edits, browseResult],
  );

  const hasMore = tab.browseResult?.truncated ?? false;
  const rowsOnPage = tab.browseResult?.rows.length ?? 0;
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs tabular-nums" style={{ color: 'var(--text-muted)' }}>
            {/* Without a zero case this read "1–0" on an empty table — a range
                running backwards, describing rows that do not exist. */}
            {rowsOnPage === 0 ? 'No rows' : `Rows ${offset + 1}–${offset + rowsOnPage}`}
          </span>
          <button
            type="button"
            className="btn btn-outline py-1"
      ...
      {!dirty && detail && !editable && !tab.readOnlyNoticeDismissed && (
        <ReadOnlyNotice
          reason={readOnlyReason(detail.kind, engine, connectionReadOnly)}
          onDismiss={() => update(tab.id, { readOnlyNoticeDismissed: true })}
        />
      )}
