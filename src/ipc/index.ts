...
export function toFaroError(e: unknown): FaroError {
  ...
  // `String(e)` is empty for a few values — `[]` and `''` among them — and an
  // empty banner is indistinguishable from no banner, so a failure would
  // render as silence. Whatever arrives, the user gets something to read.
  const message = String(e);
  return {
    kind: 'other',
    message: message.trim() === '' ? 'The operation failed without an error message.' : message,
  };
}
...
/**
 * Insert a file's rows into a table, as one transaction.
 *
 * `queryId` registers the import with the same cancellation registry a query
 * uses, so `cancelQuery(connectionId, queryId)` can stop it mid-flight — which
 * matters most for exactly the large files this is used on.
 */
export const importFile = (
  ...
  queryId: string,
) => invoke<TransferResult>('import_file', { connectionId, table, path, options, queryId });
