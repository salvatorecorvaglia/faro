...
//
// Note the asymmetry: the reason for choosing a native `<dialog>` — that the
// platform supplies focus trapping, backdrop inertness and top-layer stacking
// — is the one thing this shim cannot reproduce, so no test here proves it.
// Only Escape and the open/closed flag are real. Same conclusion as the
// virtualization note above: it needs a browser.
...
//
// WHAT THIS MEANS FOR THE TESTS BELOW IT — read before trusting a grid test.
//
// This observer never fires, and the metrics stubbed further down are fixed,
// so `@tanstack/react-virtual` measures a 600x1000 box once and never
// re-measures. Virtualization is therefore effectively *bypassed* under jsdom:
// `ResultGrid` renders a fixed window of rows whatever the real layout would
// be. A test that finds a row by its text is proving the row was rendered, not
// that the virtualizer picked the right window.
//
// So these shims cannot catch a wrong row height, a wrong overscan, or a
// window that fails to follow the scroll position. Giving the observer a real
// implementation would not help — jsdom has no layout engine for it to report,
// which is why the metrics are hard-coded in the first place. Proving any of
// that needs a real browser (vitest browser mode or Playwright), which this
// project does not have yet. The gap is deliberate and recorded here rather
// than papered over: `gridLayout.test.ts` covers the sizing arithmetic
// directly, which is the part that can be tested without a layout engine.
