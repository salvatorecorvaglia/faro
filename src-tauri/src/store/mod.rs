...
/// Narrow a path's permissions to its owner, on platforms that have them.
///
/// Passwords live in the OS keyring, not here — but hostnames, usernames and
/// database names are still a map of what this person has access to, and
/// `create_dir_all` and `Connection::open` both take whatever the umask gives
/// them, which is usually world-readable. On a shared machine that is a
/// listing of every server the user connects to.
///
/// Best effort: a filesystem that cannot express this (a mounted share, a
/// Windows path) is not a reason to refuse to start.
...
impl Store {
    pub fn open(path: &std::path::Path) -> Result<Self> {
        ...
        // After opening, so the file exists to chmod.
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn the_store_file_is_readable_only_by_its_owner() {
        // No passwords here — those are in the keyring — but the hosts,
        // usernames and database names still describe what this person can
        // reach, and the umask would otherwise leave them world-readable.
        use std::os::unix::fs::PermissionsExt;

        let path = std::env::temp_dir().join(format!("faro_perm_{}.db", std::process::id()));
        ...
        assert_eq!(mode, 0o600, "store file is reachable by other users");
    }
