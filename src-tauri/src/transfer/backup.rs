...
async fn abort_outcome(driver: &dyn Driver, atomic: bool, applied: usize, total: usize) -> String {
    ...
    match driver
        ...
        .await
    {
        Ok(_) => "Nothing was applied — the database is as it was.".to_string(),
        // Say so plainly. Believing a failed restore left no trace, when it
        // did, is worse than knowing it is half-applied.
        Err(e) => format!(
            "The rollback then failed too ({e}), so {applied} statements \
...
pub async fn restore(
    ...
    let statements: Vec<String> = sql::split_statements_with(script, driver.dialect().lex_rules())
        .into_iter()
        .filter(|s| {
            !s.lines()
