...
/// One message on the channel feeding [`crate::driver::Driver::apply_streaming`].
///
/// `Done` is explicit rather than implied by the channel closing, and that is
/// the whole point: a producer that fails partway through simply drops its
/// sender, which is indistinguishable from finishing unless the successful case
/// says so. Committing on a closed channel would commit a half-read file.
pub enum ApplyChunk {
    /// More statements to run inside the open transaction.
    Batch(Vec<GuardedStatement>),
    /// Every statement has been sent. Commit.
    Done,
}
