...
impl Dialect for SqlServerDialect {
    /// T-SQL adds `[bracketed]` identifiers, where a literal `]` is escaped by
    /// doubling. `\` has no special meaning in a literal.
    fn lex_rules(&self) -> crate::sql::LexRules {
        crate::sql::LexRules::TSQL
    }
        ...
        for (index, stmt) in statements.iter().enumerate() {
            ...
            if let Some(expected) = stmt.expect {
                if affected != expected {
                    ...
                    let guard =
                        super::tx::guard_error(index, Some(statements.len()), expected, affected);
