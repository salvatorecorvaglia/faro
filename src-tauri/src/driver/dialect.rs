...
pub trait Dialect: Send + Sync {
    ...
    /// How this engine delimits strings and identifiers.
    ///
    /// Anything that decides what a statement *does* by reading its text — the
    /// read-only guard above all — has to tokenize the way this engine will.
    /// The default is the SQL standard, which is right for Postgres, SQLite,
    /// DuckDB, CockroachDB and Redshift; the engines with their own quoting
    /// override it. Getting this wrong is not cosmetic: crediting an engine
    /// with a delimiter it does not have means skipping text it *will* execute.
    fn lex_rules(&self) -> crate::sql::LexRules {
        crate::sql::LexRules::STANDARD
    }
