...
fn build_filter(filters: &[ColumnFilter]) -> Document {
    ...
    for f in filters.iter().filter(|f| is_field_name(&f.column)) {
...
impl Driver for MongoDriver {
    ...
    async fn query(&self, sql: &str, limit: u64, cancel: CancellationToken) -> Result<ResultSet> {
        ...
        // Always the connection's own database, where `browse` honours the
        // schema picked in the sidebar.
        //
        // Not an oversight but a limitation with nowhere to put the fix yet:
        // `Driver::query` receives only the query text, so there is no schema
        // to honour. A query tab against a non-default database therefore needs
        // that database to be the connection's. Closing this means either a
        // schema argument on `query` or a `use <db>` form in `parse_query`.
    ...
    async fn browse(
        ...
        // Same rule as the filters: a `$`-prefixed key is an operator here.
        if let Some(column) = options.sort_column.as_deref().filter(|c| is_field_name(c)) {
...
/// Whether `name` addresses a document field rather than a query operator.
///
/// The SQL path checks every filter and sort column against the table's real
/// columns before interpolating it, because a name there becomes SQL. The same
/// reasoning applies to a query document: a key beginning with `$` is an
/// *operator* at the position these are inserted, not a field, so a collection
/// whose inferred fields included one would turn a filter into `$where` or
/// `$expr`. MongoDB does not permit a stored field name to begin with `$`
...
mod tests {
    ...
    #[test]
    fn a_dollar_prefixed_name_is_dropped_rather_than_becoming_an_operator() {
        // At this position in the document a `$` key is an operator, not a
        // field — the same reason the SQL path checks filter columns against
        // the table's real columns before interpolating them.
        let doc = build_filter(&filter("$where", FilterOp::Equals, "1"));
        assert!(doc.is_empty(), "operator key survived: {doc:?}");

        let doc = build_filter(&filter("$expr", FilterOp::Contains, "x"));
        assert!(doc.is_empty(), "operator key survived: {doc:?}");

        // Ordinary names, including dotted paths into a subdocument, are fine.
        assert!(is_field_name("name"));
        assert!(is_field_name("address.city"));
        assert!(!is_field_name("$where"));
        assert!(!is_field_name(""));
    }

    #[test]
    fn a_valid_filter_beside_a_rejected_one_still_applies() {
        let mut filters = filter("age", FilterOp::Equals, "30");
        filters.extend(filter("$where", FilterOp::Equals, "1"));

        let doc = build_filter(&filters);
        assert_eq!(doc.get("age"), Some(&Bson::Int32(30)));
        assert_eq!(doc.len(), 1, "operator key survived: {doc:?}");
    }
