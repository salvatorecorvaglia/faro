...
use crate::model::{
    ApplyChunk, BrowseOptions, ConnectionConfig, Engine, ExecResult, GuardedStatement,
    QueryOutcome, ResultSet, SchemaInfo, TableColumns, TableDetail, TableInfo, TableRef,
};
...
pub trait Driver: Send + Sync {
    ...
    /// Apply statements that arrive over time, all inside one transaction.
    ///
    /// For import, where the statements are generated from a file being read in
    /// bounded batches. [`apply_transaction`](Self::apply_transaction) takes a
    /// finished slice, so using it means holding every statement — and so the
    /// whole file, as SQL text — in memory before the first one runs. The
    /// channel's bound is what actually caps that: the reader stops once the
    /// applier falls behind.
    ///
    /// Commits only on [`ApplyChunk::Done`]. A sender dropped without it means
    /// the producer gave up, so the transaction rolls back.
    ///
    /// The default buffers and delegates, which is correct but gives up the
    /// memory bound; the sqlx-backed engines override it. An engine that cannot
    /// stream is no worse off than before.
    async fn apply_streaming(
        &self,
        mut chunks: tokio::sync::mpsc::Receiver<ApplyChunk>,
    ) -> Result<u64> {
        let mut all = Vec::new();
        let mut done = false;
        while let Some(chunk) = chunks.recv().await {
            match chunk {
                ApplyChunk::Batch(batch) => all.extend(batch),
                ApplyChunk::Done => {
                    done = true;
                    break;
                }
