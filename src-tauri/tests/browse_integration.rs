...
macro_rules! driver_or_skip {
    () => {
        match open().await {
            Some(d) => d,
            None => {
                // `FARO_REQUIRE_LIVE=1` says the fixture is supposed to be
                // there, so its absence is a broken setup rather than a
                // machine that simply has not seeded one. Without this, a
                // fresh clone reports every test here as passing.
                assert!(
                    !std::env::var("FARO_REQUIRE_LIVE").is_ok_and(|v| !v.is_empty() && v != "0"),
                    "FARO_REQUIRE_LIVE is set but the fixture is missing; run ./scripts/seed.sh"
                );
